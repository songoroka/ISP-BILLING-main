...
class PackageListSetup extends Component
{
    ...
    public $validity_type = null;

    public $validity_duration = null;

    public $ttl_mangle_enabled = false;
public $ttl_value = 1;
public $ttl_chain = 'postrouting';
    ...
    public function mount(): void
    {
        ...
        $this->reset([
            ...
            'mikrotik_rate_limit', 'mikrotik_local_address', 'mikrotik_remote_address',
                'push_to_mikrotik', 'service_type', 'router_name',
                'validity_type', 'validity_duration',
                'ttl_mangle_enabled', 'ttl_value', 'ttl_chain',
        ]);
        $this->show_on_site = true;
            $this->ttl_mangle_enabled = false;
            $this->ttl_value = 1;
            $this->ttl_chain = 'postrouting';
        $this->dataRender();
    }
    ...
    protected function rules(): array
    {
        return [
            ...
            'service_type' => 'required|in:ppp,hotspot,both',
            'validity_type' => 'nullable|in:realtime',
            'validity_duration' => [
                'nullable',
                'string',
                'max:50',
            ...
            'ttl_value' => 'nullable|integer|min:1|max:255',
            'ttl_chain' => 'required_if:ttl_mangle_enabled,true|in:prerouting,input,forward,output,postrouting',
        ];
    }
        ...
        try {
            ...
            $package = PackageList::updateOrCreate(
                ...
                [
                    ...
                    'service_type' => $this->service_type ?: 'both',
                    'validity_type' => $this->validity_type ?: null,
                    'validity_duration' => $this->validity_duration ?: null,
                    'router_name' => $normalizedRouterName,
                    'ttl_mangle_enabled' => $this->ttl_mangle_enabled,
                    'ttl_value' => $this->ttl_mangle_enabled ? ($this->ttl_value ?: 1) : null,
                    'ttl_chain' => $this->ttl_chain ?: 'postrouting',
                ]
            );
            ...
            //   ppp     = PPP profile only
            //   hotspot = Hotspot profile + optional TTL/Mangle
            //   both    = PPP + Hotspot
            //
            $pushError = null;

            $newServiceType = $this->service_type ?: 'both';
            $newHasPpp = in_array($newServiceType, ['ppp', 'both'], true);
            $newHasHotspot = in_array($newServiceType, ['hotspot', 'both'], true);

            $oldServiceType = $oldPackage?->service_type ?: 'both';
            $oldHasPpp = in_array($oldServiceType, ['ppp', 'both'], true);
            $oldHasHotspot = in_array($oldServiceType, ['hotspot', 'both'], true);

            \Log::debug(
                'PackageListSetup: MikroTik service sync: '
                .'service_type='.$newServiceType
                .', PPP='.($newHasPpp ? 'YES' : 'NO')
                .', Hotspot='.($newHasHotspot ? 'YES' : 'NO')
                .', push='.($this->push_to_mikrotik ? 'TRUE' : 'FALSE')
                .', router='.($normalizedRouterName ?? 'ALL')
            );

            /*
             * -------------------------------------------------------------
             * 2A. CLEAN UP OLD CONFIGURATION
             * -------------------------------------------------------------
             */
            if ($oldPackage && $oldPackage->push_to_mikrotik) {
                $oldRouter = $oldPackage->router_name;
                $routerChanged = $oldRouter !== $normalizedRouterName;
                $renamed = $oldPackage->package !== $this->package_name;

                // If PPP is no longer required, or push/router changed,
                // remove the old PPP profile.
                if (
                    $oldHasPpp
                    && (
                        ! $this->push_to_mikrotik
                        || ! $newHasPpp
                        || $routerChanged
                    )
                ) {
                    \Log::info(
                        "PackageListSetup: Removing old PPP profile '{$oldPackage->package}'"
                        .($oldRouter ? " from router '{$oldRouter}'" : ' from configured routers')
                    );

                    $controller->deleteProfileFromRouters(
                        $oldPackage->package,
                        $oldRouter
                    );
                }

                // If Hotspot is no longer required, or push/router changed,
                // remove the old Hotspot profile and its TTL rule.
                if (
                    $oldHasHotspot
                    && (
                        ! $this->push_to_mikrotik
                    ...
                    \Log::info(
                        "PackageListSetup: Removing old Hotspot configuration '{$oldPackage->package}'"
                        .($oldRouter ? " from router '{$oldRouter}'" : ' from configured routers')
                    );

                    if ($oldRouter) {
                        $controller->deleteHotspotUserProfile(
                            $oldRouter,
                            $oldPackage->package
                        );

                        $controller->removeHotspotTtlMangle(
                            $oldRouter,
                            $oldPackage->package
                        );
                    } else {
                        foreach (RouterList::where('action', 'connected')->get() as $router) {
                            $controller->deleteHotspotUserProfile(
                                $router->router_name,
                                $oldPackage->package
                            );
                ...
                // If the package was renamed while remaining a PPP package,
                // rename the existing PPP profile.
                if (
                    $oldHasPpp
                    && $newHasPpp
                    && $this->push_to_mikrotik
                    ...
                    \Log::info(
                        "PackageListSetup: Renaming PPP profile '{$oldPackage->package}'"
                        ." to '{$this->package_name}'"
                    );

                    $controller->updateProfileOnRouters(
                        $oldPackage->package,
                        $this->package_name,
                        $this->mikrotik_rate_limit,
                        $this->mikrotik_local_address,
                        $this->mikrotik_remote_address,
            ...
            /*
             * -------------------------------------------------------------
             * 2B. SYNCHRONIZE CURRENT PPP CONFIGURATION
             * -------------------------------------------------------------
             */
            if ($this->push_to_mikrotik && $newHasPpp) {
                try {
                    \Log::info(
                        "PackageListSetup: Syncing PPP profile '{$this->package_name}'"
                        ." on router: ".($normalizedRouterName ?? 'ALL')
                    );

                    $pppResults = $controller->pushProfileToRouters(
                        $this->package_name,
                        $this->mikrotik_rate_limit ?: null,
                        $this->mikrotik_local_address ?: null,
                        $this->mikrotik_remote_address ?: null,
                        $normalizedRouterName
                    );

                    \Log::debug(
                        'PackageListSetup: PPP sync result: '
                        .json_encode($pppResults)
                    );

                    $pppFailures = [];

                    foreach ($pppResults as $routerKey => $result) {
                        if (
                            is_string($result)
                            && str_starts_with($result, 'Error:')
                        ) {
                            $pppFailures[] = $routerKey.': '.$result;
                        }
                    }

                    if (! empty($pppFailures)) {
                        $pushError = 'PPP profile sync failed: '.implode('; ', $pppFailures);
                    }
                } catch (\Throwable $e) {
                    \Log::error(
                        "PackageListSetup: PPP sync failed for '{$this->package_name}': "
                        .$e->getMessage(),
                        ['exception' => $e]
                    );

                    $pushError = 'PPP profile sync failed: '.$e->getMessage();
                }
            }

            /*
             * -------------------------------------------------------------
             * 2C. SYNCHRONIZE CURRENT HOTSPOT CONFIGURATION
             * -------------------------------------------------------------
             */
            if ($this->push_to_mikrotik && $newHasHotspot) {
                \Log::info(
                    "PackageListSetup: Syncing Hotspot profile '{$this->package_name}'"
                    ." on router: ".($normalizedRouterName ?? 'ALL')
                );

                try {
                    if ($normalizedRouterName) {
                        $hotspotResults = $controller->syncHotspotProfilesToRouter(
                            $normalizedRouterName,
                            collect([$package])
                        );
                    } else {
                        ...
                        foreach (RouterList::where('action', 'connected')->get() as $router) {
                            $routerResults = $controller->syncHotspotProfilesToRouter(
                                $router->router_name,
                                collect([$package])
                            );

                            $hotspotResults[$router->router_name] = $routerResults;
                        }
                    }

                    \Log::debug(
                        'PackageListSetup: Hotspot sync result: '
                        .json_encode($hotspotResults)
                    );

                    $hotspotFailures = [];

                    foreach ($hotspotResults as $routerKey => $result) {
                        if (is_array($result)) {
                            foreach ($result as $profileName => $status) {
                                if (
                                    is_string($status)
                                    && str_starts_with($status, 'Error:')
                                ) {
                                    $hotspotFailures[] = $routerKey.': '.$status;
                                }
                            }
                        } elseif (
                            is_string($result)
                            ...
                            $hotspotFailures[] = $routerKey.': '.$result;
                        }
                    }

                    if (! empty($hotspotFailures)) {
                        $message = 'Hotspot profile sync failed: '.implode('; ', $hotspotFailures);
                    ...
                    \Log::error(
                        "PackageListSetup: Hotspot sync failed for '{$this->package_name}': "
                        .$e->getMessage(),
                        ['exception' => $e]
                    );

                    $message = 'Hotspot profile sync failed: '.$e->getMessage();
    ...
    public function editPackage(int $id): void
    {
        ...
        $this->service_type = $package->service_type ?: 'both';
        $this->validity_type = $package->validity_type;
        $this->validity_duration = $package->validity_duration;
        $this->ttl_mangle_enabled = (bool) $package->ttl_mangle_enabled;
        $this->ttl_value = $package->ttl_value ?: 1;
        $this->ttl_chain = $package->ttl_chain ?: 'postrouting';
