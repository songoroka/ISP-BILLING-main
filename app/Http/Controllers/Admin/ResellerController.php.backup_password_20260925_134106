...
class ResellerController extends Controller
{
    ...
    public function create()
    {
        ...
        // Only allow non-system roles to be assigned in the reseller form.
        // Super Admin, Admin, Reseller and Hotel User remain protected portal roles.
        $availableRoles = Role::where('guard_name', 'web')
            ->whereNotIn('name', ['Super Admin', 'Admin', 'Reseller', 'Hotel User'])
            ->orderBy('name')
            ->get();

        // Get the reseller's currently assigned optional roles.
        // Protected portal roles are intentionally excluded.
        $assignedAdditionalRoles = $reseller->user->getRoleNames()
            ->reject(function ($role) {
                return in_array($role, ['Super Admin', 'Admin', 'Reseller', 'Hotel User'], true);
            })
            ->values()
            ->toArray();

        return view('admin.resellers.create', compact(
            'packages',
            'resellerPermissions',
            'defaultPermissions',
            'availableRoles'
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'routers.*' => 'integer|exists:router_lists,id',
    ...
    public function edit(Reseller $reseller)
    {
        ...
        // Only allow non-system roles to be assigned in the reseller edit form.
        // Super Admin, Admin, Reseller and Hotel User remain protected portal roles.
        $availableRoles = Role::where('guard_name', 'web')
            ->whereNotIn('name', ['Super Admin', 'Admin', 'Reseller', 'Hotel User'])
            ->orderBy('name')
        ...
        // Available MikroTik routers for reseller assignment.
        $availableRouters = RouterList::orderBy('router_name')->get();

        // Routers currently assigned to this reseller.
        $assignedRouterIds = $reseller->assignedRouters()
            ->pluck('router_lists.id')
            ->toArray();

        // Get currently assigned optional roles for this reseller.
        // Protected portal roles are excluded from this optional-role selector.
        $assignedAdditionalRoles = $reseller->user->getRoleNames()
            ->reject(function ($role) {
                return in_array($role, ['Super Admin', 'Admin', 'Reseller', 'Hotel User'], true);
            })
        ...
        return view('admin.resellers.edit', compact(
            ...
            'resellerPermissions', 'userPermissions', 'availableRoles',
            'assignedAdditionalRoles', 'availableRouters', 'assignedRouterIds',
            'walletTransactions', 
    ...
    public function update(Request $request, Reseller $reseller)
    {
        ...
        try {
            ...
            // Sync MikroTik routers assigned to this reseller.
            $selectedRouters = $request->input('routers', []);
            $reseller->assignedRouters()->sync($selectedRouters);
            ...
            $logMessage .= " and " . count($selectedRouters) . " assigned routers";
