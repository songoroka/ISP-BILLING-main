...
class MikrotikController extends Controller
{
     ...
     */
    protected function getPooledSshService(string $ip, int $port, string $username, string $password, int $timeout = 20): MikrotikSSHService
    {
        $key = "{$ip}:{$port}:{$username}";
    ...
    {
        $sensitive = str_contains($path, 'secret') ? ' show-sensitive' : '';
    ...
    public function addHotspotUser(string $routerName, array $p, ?string $oldName = null): string
    {
        ...
        if (! empty($p['mac-address'])) {
            $mac = strtoupper(trim($p['mac-address']));
            ...
            if (! preg_match(
                ...
                throw new \InvalidArgumentException(
                    'Invalid Hotspot MAC address: '.$mac
                );
            }

            $kv[] = 'mac-address='.$this->mtQuote($mac);
        }
    ...
    public function addHotspotUserProfile(string $routerName, array $p, ?string $oldName = null): string
{
    ...
    if (! empty($p['address_list'])) {
        $kv[] = 'address-list='.$this->mtQuote($p['address_list']);
    }
    ...
    if ($oldName) {
        $cmd = '/ip hotspot user profile set [find name='.$this->mtQuote($oldName).']';
        ...
        $cmd = '/ip hotspot user profile add name='.$this->mtQuote($name);
    ...
    \Log::debug(
        'HotspotUserProfile '.($oldName ? "SET[{$oldName}]" : 'ADD')." CMD: {$cmd}"
    );
    ...
    /**
     * Build a deterministic MikroTik address-list name for a package.
     */
    public function hotspotTtlAddressList(string $packageName): string
    {
        $safe = preg_replace('/[^A-Za-z0-9_-]+/', '-', trim($packageName));
        $safe = trim($safe, '-_');
        ...
        return 'HOTSPOT-TTL-'.$safe;
    }

    /**
     * Create or update the package-specific Hotspot TTL mangle rule.
     */
    public function syncHotspotTtlMangle(
        string $routerName,
        string $packageName,
        int $ttl,
        string $chain = 'postrouting'
    ): string {
        $ttl = max(1, min(255, $ttl));
        $chain = in_array($chain, ['prerouting', 'input', 'forward', 'output', 'postrouting'], true)
            ? $chain
            : 'postrouting';

        $addressList = $this->hotspotTtlAddressList($packageName);

        // Remove an older rule for this exact package/list first.
        $removeCmd = '/ip firewall mangle remove [find src-address-list='.$this->mtQuote($addressList).' action=change-ttl]';
        $this->singleWrite($routerName, $removeCmd);

        // Add the new package-specific rule.
        $cmd = '/ip firewall mangle add'
            .' chain='.$chain
            .' src-address-list='.$this->mtQuote($addressList)
            .' action=change-ttl'
            .' new-ttl=set:'.$ttl;
    ...
    /**
     * Remove the package-specific Hotspot TTL mangle rule.
     */
    public function removeHotspotTtlMangle(
        string $routerName,
        string $packageName
    ): string {
        $addressList = $this->hotspotTtlAddressList($packageName);
        ...
        return $this->singleWrite(
            $routerName,
            '/ip firewall mangle remove [find src-address-list='.$this->mtQuote($addressList).' action=change-ttl]'
        );
    }
    ...
    public function syncHotspotProfilesToRouter(string $routerName, $packages): array
    {
        ...
        foreach ($packages as $pkg) {
            try {
                ...
                // When TTL mangle is enabled, the Hotspot profile places
                // logged-in users into a package-specific address list.
                if ($pkg->ttl_mangle_enabled) {
                    $p['address_list'] = $this->hotspotTtlAddressList($pkg->package);
                }
                ...
                // Keep the package-specific TTL rule synchronized.
                if ($pkg->ttl_mangle_enabled) {
                    $this->syncHotspotTtlMangle(
                        $routerName,
                        $pkg->package,
                        (int) ($pkg->ttl_value ?: 1),
                        $pkg->ttl_chain ?: 'postrouting'
                    );
                } else {
                    $this->removeHotspotTtlMangle(
                        $routerName,
                        $pkg->package
                    );
                }
