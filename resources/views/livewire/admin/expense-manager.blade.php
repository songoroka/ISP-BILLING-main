    ...
    <div class="container-fluid py-4">
                            ...
                            @foreach($categories as $key => $label)
                                @if(isset($totals[$key]) && $totals[$key] > 0)
                                    ...
                                    <span class="badge rounded-pill px-3 py-2 small fw-semibold
                                        ...
                                        style="{{ $style }}">
                                        {{ __($label) }}: TSh{{ number_format($totals[$key], 2) }}
                                    </span>
                                @endif
                            @endforeach
                        </div>
        ...
        <div class="alert rounded-4 border-0 mb-4 py-3 px-4 d-flex align-items-center"
            ...
            <div>
                ...
                <span class="fw-bold text-danger fs-5 ms-2">TSh{{ number_format($grandTotal, 2) }}</span>
            </div>
        </div>
        @endif
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr wire:key="exp-{{ $expense->id }}">
                                ...
                                <td class="text-end fw-bold text-danger">TSh{{ number_format($expense->amount, 2) }}</td>
            ...
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-body px-4 pt-3">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <select wire:model.live="linkedResellerId"
                                ...
                                @foreach($resellers as $r)
                                    <option value="{{ $r->id }}">
                                        ...
                                        — {{ __('Balance:') }} TSh{{ number_format($r->balance, 2) }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @if($linkedResellerId)
                                ...
                                <div class="alert alert-warning rounded-3 py-2 px-3 small mt-2 mb-0">
                                    ...
                                    {!! __('Amount <strong>TSh:amount</strong> exceeds reseller balance <strong>TSh:balance</strong>. Save will fail.', ['amount' => number_format((float)$amount, 2), 'balance' => number_format($selReseller->balance, 2)]) !!}
                                </div>
                                @elseif($selReseller)
                                <div class="text-muted small mt-1">
                                    <i class="bi bi-wallet2 me-1"></i>
                                    {{ __('Current wallet balance:') }} <strong class="text-success">TSh{{ number_format($selReseller->balance, 2) }}</strong>
                                    @if($amount)
                                        → {{ __('After payout:') }} <strong class="text-danger">TSh{{ number_format(max(0,(float)$selReseller->balance - (float)$amount), 2) }}</strong>
                                    @endif
                                </div>
                                @endif
                            @endif
                        ...
                        {{-- Amount --}}
                        <div class="col-md-4">
                            <label class="form-label small fw-semibold">{{ __('Amount') }} (TSh) <span class="text-danger">*</span></label>
