@if (auth()->user()->roles()->exists() || auth()->user()->permissions()->exists())
    @if (auth()->user()->hasRole('Hotel User'))
        <ul class="navbar-nav flex-column mb-3" id="navbarVerticalNav">
            <li class="nav-item">
                <div class="row navbar-vertical-label-wrapper mt-3 mb-2">
                    <div class="col-auto navbar-vertical-label">{{ __('Hotel') }}</div>
                    <div class="col ps-0"><hr class="mb-0 navbar-vertical-divider" /></div>
                </div>
                @can('hotel-view-guests')
                    <a wire:navigate.hover wire:current="active" class="nav-link" href="{{ route('hotel.guests') }}" role="button">
                        <div class="d-flex align-items-center">
                            <span class="nav-link-icon"><i class="bi bi-building"></i></span>
                            <span class="nav-link-text ps-1">{{ __('Guest Registration') }}</span>
                        </div>
                    </a>
                @endcan
                @can('hotel-create-guest')
                    <a wire:navigate.hover wire:current="active" class="nav-link" href="{{ route('hotel.guests') }}" role="button">
                        <div class="d-flex align-items-center">
                            ...
                            <span class="nav-link-text ps-1">{{ __('Register New Guest') }}</span>
                        </div>
                    </a>
                @endcan
                <a wire:navigate.hover wire:current="active" class="nav-link" href="{{ route('profile.show') }}" role="button">
                    <div class="d-flex align-items-center">
                        <span class="nav-link-icon"><i class="bi bi-person-circle"></i></span>
                        <span class="nav-link-text ps-1">{{ __('My Profile') }}</span>
                    </div>
                </a>
            </li>
        </ul>
    @elseif (currentReseller())
        <ul class="navbar-nav flex-column mb-3" id="navbarVerticalNav">
            <li class="nav-item px-3 pt-2 pb-1">
                <div class="small fw-bold text-primary">SKYTECH INFRANET</div>
                <div class="small text-muted text-truncate" title="{{ currentReseller()?->company ?: currentReseller()?->user?->name }}">{{ currentReseller()?->company ?: currentReseller()?->user?->name }}</div>
            </li>
            ...
            <li class="nav-item">
                ...
                @can('hotel-view-guests')
                    ...
                    <a wire:navigate.hover wire:current="active" class="nav-link" href="{{ route('hotel.guests') }}" role="button">
                        <div class="d-flex align-items-center">
                            <span class="nav-link-icon"><i class="bi bi-building"></i></span>
                            <span class="nav-link-text ps-1">{{ __('Hotel Guest Registration') }}</span>
                        </div>
                    </a>
                @endcan
