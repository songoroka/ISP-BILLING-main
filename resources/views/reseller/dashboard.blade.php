...
<div class="alert alert-primary border-0 shadow-sm rounded-3 mb-4"><div class="d-flex align-items-center justify-content-between flex-wrap gap-2"><div><div class="small text-uppercase fw-bold opacity-75">SKYTECH INFRANET</div><div class="h5 mb-0 fw-bold">{{ currentReseller()?->company ?: currentReseller()?->user?->name ?: 'Reseller Portal' }}</div><div class="small text-muted">{{ currentReseller()?->phone ?: currentReseller()?->user?->mobile }}</div></div></div></div>
                ...
                <div class="card-body position-relative z-1">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="fw-bold mb-0">TSh{{ number_format($walletBalance, 2) }}</h3>
                        </div>
                ...
                <div class="card-body position-relative z-1">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="fw-bold mb-0">TSh{{ number_format($earningsMonth, 2) }}</h3>
                        </div>
                        <div class="rounded-circle d-flex align-items-center justify-content-center" style="width: 48px; height: 48px; background: rgba(255,255,255,0.15);">
                    ...
                    <div class="mt-3 pt-2 border-top border-white border-opacity-10 d-flex justify-content-between small text-white-50">
                        <span>{{ __('Today:') }} TSh{{ number_format($earningsToday, 2) }}</span>
                        <span>{{ __('Total:') }} TSh{{ number_format($totalEarnings, 2) }}</span>
                    </div>
                </div>
                <div class="position-absolute end-0 bottom-0" style="opacity: 0.08; transform: translate(10%, 10%); z-index: 0; pointer-events: none;">
                        ...
                        @forelse($packageSales as $sale)
                            <div class="list-group-item d-flex justify-content-between align-items-center py-2.5 px-3">
                                <div>
                                    ...
                                    <small class="text-muted">{{ __('Speed:') }} {{ $sale->package->speed ?? 'N/A' }} | {{ __('Price:') }} TSh{{ number_format($sale->package->price ?? 0, 0) }}</small>
                                </div>
            ...
            function initChart() {
                ...
                if (chartEl) {
                    ...
                    const options = {
                        ...
                        yaxis: {
                            labels: {
                                formatter: function (val) { return 'TSh' + val.toFixed(0); },
                                style: { colors: '#94a3b8' }
                            }
                        },
                        tooltip: {
                            y: {
                                formatter: function (val) { return 'TSh' + val.toFixed(2); }
                            }
                        }
                    };
