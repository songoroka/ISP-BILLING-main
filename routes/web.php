...
use App\Livewire\Hotel\GuestManager;
use App\Http\Controllers\Hotel\HotelVoucherController;
...
$portalDomain = env('FORTIFY_DOMAIN', 'portal.'.$baseDomain);
$resellerDomain = env('RESELLER_DOMAIN', 'reseller.'.$baseDomain);
$demoDomain = env('DEMO_DOMAIN', 'demo.'.$baseDomain);
$hotelDomain = env('HOTEL_DOMAIN', 'hotel.'.$baseDomain);

// Public MikroTik Hotspot captive portal.
// MikroTik redirects unauthenticated clients here with router/MAC/IP context.
Route::get('/hotspot/portal', [\App\Http\Controllers\Hotspot\HotspotPortalController::class, 'show'])
    ->name('hotspot.portal');

// Catch-all redirect for external domains or direct IP requests (NAT Redirection for Expired Users)
$currentHost = request()->getHost();
if ($currentHost && !in_array($currentHost, [$baseDomain, 'www.'.$baseDomain, $adminDomain, $portalDomain, $resellerDomain, $demoDomain, $hotelDomain])) {
...
// Main domain
Route::domain('www.'.$baseDomain)->group(function () use ($baseDomain) {
    Route::any('{any?}', function () use ($baseDomain) { return redirect()->away('https://'.$baseDomain.request()->getRequestUri()); })->where('any', '.*');
});
...
Route::domain($baseDomain)->group(function () {
    ...
    Route::get('/portal', function () {
        ...
        return redirect()->away('https://'.env('ADMIN_PORTAL_DOMAIN', 'portal.'.$baseDomain));
    });
...
// Hotel users authenticate through the central portal, then work from the hotel subdomain.
Route::domain($hotelDomain)->group(function () use ($portalDomain) {
    Route::get('/login', function () use ($portalDomain) {
        return redirect()->away('https://'.$portalDomain.'/login');
    })->name('hotel.login');
});
...
Route::middleware([
    ...
    'verified',
    'restrict.profile',
])->group(function () use ($baseDomain, $adminDomain, $hotelDomain, $portalDomain) {

    // Dedicated hotel workspace domain
    Route::domain($hotelDomain)->middleware(['force.portal.password', 'hotel.portal'])->group(function () {
        Route::redirect('/', '/hotel/guests');

        Route::get('/hotel/guests', GuestManager::class)
            ->middleware('role:Hotel User|Super Admin')
            ->name('hotel.guests');
        Route::get('/hotel/guests/{guest}/print', [HotelVoucherController::class, 'print'])
            ->middleware('role:Hotel User|Super Admin')
            ->name('hotel.guests.print');
    });

    // Super Admin management domain
    Route::domain($adminDomain)
        ->middleware(['force.portal.password', 'hotel.portal', 'auth:web'])
        ...
        Route::prefix('admin')->name('admin.')->group(function () {
            ...
            Route::post('resellers/exit', [ResellerController::class, 'exitPortal'])->name('resellers.exit');
...
// Dedicated reseller domain routes
Route::domain($resellerDomain)->middleware(['web', 'reseller'])->name('reseller.')->group(function () {
    Route::get('/dashboard', [ResellerDashboardController::class, 'index'])->name('dashboard');

    Route::post('/exit-context', [ResellerController::class, 'exitPortal'])
...
Route::domain($portalDomain)->group(function () {
    // Customer web panel has been retired. Public callbacks/webhooks remain.
    ...
    Route::post('/payment/tanzania/selcom/webhook', [\App\Http\Controllers\Payment\TanzaniaPaymentController::class, 'selcomWebhook'])->name('payment.tanzania.selcom.webhook');
    Route::post('/payment/tanzania/mpesa/webhook', [\App\Http\Controllers\Payment\TanzaniaPaymentController::class, 'mpesaWebhook'])->name('payment.tanzania.mpesa.webhook');
    Route::post('/payment/tanzania/beem-bpay/webhook', [\App\Http\Controllers\Payment\TanzaniaPaymentController::class, 'beemBpayWebhook'])->name('payment.tanzania.beem_bpay.webhook');
    Route::post('/payment/tanzania/azampesa/webhook', [\App\Http\Controllers\Payment\TanzaniaPaymentController::class, 'azampesaWebhook'])->name('payment.tanzania.azampesa.webhook');
...
// Demo portal: only accounts explicitly granted demo_access by Super Admin may enter.
Route::domain($demoDomain)->middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified', 'demo.access', 'force.portal.password'])->group(function () {
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('demo.dashboard');
    Route::get('/reseller/dashboard', [ResellerDashboardController::class, 'index'])->middleware('reseller')->name('demo.reseller.dashboard');
});
